...
public class AReportExecutor implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command arg1, String arg2, String[] args) {
		if(sender instanceof Player) {
		    ...
		    if(reports.containsKey(p.getName())) {
		    	p.sendMessage(Main.getInstance().config.cfg.getString("apanel.areport-have-report").replaceAll("&", "§"));
			} else if(now.longValue() - was.longValue() < 60) {
		    	String mess = Main.getInstance().config.cfg.getString("apanel.areport-cooldown");
		    	mess = mess.replaceAll("%time%", "" + (60 - (now.longValue() - was.longValue())));
		    	mess = mess.replaceAll("&", "§");
		    	p.sendMessage(mess);
		    } else {
		    	p.sendMessage(Main.getInstance().config.cfg.getString("apanel.areport-success").replaceAll("&", "§"));
